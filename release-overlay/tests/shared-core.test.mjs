...
test("SNAP and Cash reveal only scanning first, with Cash immediately after SNAP", () => {
  const modesStart = minimalFlow.indexOf("function minimalShopModesHtml(d)");
  const modesEnd = minimalFlow.indexOf("function minimalQuantityStepper", modesStart);
  const modes = minimalFlow.slice(modesStart, modesEnd);
  assert.ok(modes.indexOf("modes.push(['SNAP'") < modes.indexOf("modes.push(['CASH'"));
  assert.ok(modes.indexOf("modes.push(['CASH'") < modes.indexOf("modes.push(['WIC'"));

  const entryStart = minimalFlow.indexOf("function minimalSnapCashEntryHtml(d)");
  const readyStart = minimalFlow.indexOf("const manual=", entryStart);
  const initialEntry = minimalFlow.slice(entryStart, readyStart);
  assert.ok(initialEntry.includes('data-action="scan-barcode"'));
  assert.ok(!initialEntry.includes('id="itemInput"'));
  assert.ok(!initialEntry.includes('id="quantityInput"'));
  assert.ok(!initialEntry.includes('id="shopPriceInput"'));
  assert.ok(minimalFlow.includes("cancel:()=>{const d=ensureShopDraft();d.entryMode='IDLE'"));
});
...
test("a successful scan shows only the food name above quantity and price", () => {
  const entryStart = minimalFlow.indexOf("function minimalSnapCashEntryHtml(d)");
  const entryEnd = minimalFlow.indexOf("function compactBasketValidationHtml", entryStart);
  const entry = minimalFlow.slice(entryStart, entryEnd);
  assert.ok(entry.includes("<div class=\"input-like\">${esc(name)}</div>"));
  assert.ok(!entry.includes("USDA FoodData Central"));
  assert.ok(!entry.includes("d.category"));
  assert.ok(!entry.includes("nativeRecord.source"));
});
...
test("all shopping quantities use plus and minus controls while the date stays automatic", () => {
  assert.ok(minimalFlow.includes("minimalQuantityStepper(d.quantityRaw)"));
  assert.ok(minimalFlow.includes("minimalQuantityStepper(quantity,'wic-qty-minus','wic-qty-plus'"));
  assert.ok(html.includes("a==='qty-minus'||a==='qty-plus'"));
  assert.ok(minimalFlow.includes("action==='wic-qty-minus'||action==='wic-qty-plus'"));
  assert.ok(minimalFlow.includes("state.basket.transactionDate=today()"));
  assert.ok(!minimalFlow.includes('id="shopTransactionDate"'));
});
...
test("numeric fields are routed through one in-app keypad", () => {
  ...
  assert.ok(html.includes('data-action="money-pad-cents" data-cents="00">.00</button>'));
...
test("native iOS scanner copy is complete in English and Puerto Rican Spanish", () => {
  const match = nativeApp.match(/const SCANNER_COPY = (\{[\s\S]*?\n\}) as const;/);
  ...
  const referencedKeys = new Set(
    [...nativeApp.matchAll(/\b(?:copy|scannerCopy)\.([a-zA-Z0-9_]+)/g)]
...
test("reviewed UI safeguards remain wired into the canonical source", () => {
  ...
  assert.ok(scannerApp.includes("useCameraPermissions()"));
  assert.ok(nativeApp.includes("React.lazy("));
  assert.ok(nativeApp.includes('import("./src/BarcodeScannerCamera")'));
